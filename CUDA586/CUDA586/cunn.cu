...
void CUNN::train(const Matrix trainingData, const Vector trainingLabels, const int iterations,
               const int batchSize, float learningRate) {
  // TODO
}

void CUNN::updateFromBatch(const Matrix batch, const Vector labels, const float learningRate) {
...
void CUNN::backwards(std::vector<Matrix> &dWeights_output, std::vector<Vector> &dBiases_output, 
...
void CUNN::cost_derivative(const Vector &last_activation, const int label, Vector &result){
  for (int i=0; i < 10; i++){
      if (i == label){
...
void CUNN::activation_derivative(const Matrix &weights, Vector &z, Vector &previous){
  d_sigmoid(z);
...
float CUNN::evaluate(const Matrix &testData, const std::vector<int> &testLabels) {
  int numCorrect = 0;
...
Vector &CUNN::forwardZ(const Matrix &w, const Vector &b, const Vector &a, Vector &result){
...
Vector &CUNN::multiply_elementwise(const Vector &a, const Vector &b, Vector &result){
  for (int i = 0; i < a.size(); i++){
    result[i] = a[i]*b[i];
  }
  return result;
...
Vector &CUNN::add(const Vector &x, const Vector &b, Vector &result, const float scale) {
...
Matrix &CUNN::add(const Matrix &x, const Matrix &b, Matrix &result, const float scale) {
  assert(x.size() == b.size() && x.size() == result.size());
...
{
    return cu_utility::cuDSigmoid(x);
}
